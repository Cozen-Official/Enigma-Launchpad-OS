...
- **ProTV panels inside hidden menus** — avoid nesting ProTV UI panels (such as **MediaControls**) inside the Flatline admin menu or any other container that starts the world hidden. ProTV panels that are inactive while the TV starts up can come back with a blank track title and time display. Keep media-control panels somewhere that is active at world load, and use the whitelist to gate *interaction* rather than visibility. Enigma OS also protects against this: it never hides the admin menu before it has granted it once in a session, and when it re-shows the menu it asks ProTV to refresh its panels.
